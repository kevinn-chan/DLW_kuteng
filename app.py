...
#base ui
st.set_page_config(page_title="Traffic Engine", page_icon="🚦", layout="wide")
st.title("🚦Autonomous Traffic Incident Engine")
...
#sidebar for environment contexts
...
with col1:
    ...
    video_file = open('assets/trafficcropped.MP4', 'rb')
    video_bytes = video_file.read()
    stop_button = st.button("🛑Stop Stream")

    if video_file is not None and not stop_button:
        ...
        if "gate" not in st.session_state:
            st.session_state.gate = risk_engine.PersistenceGate(consecutive=2)
        st.session_state.gate.reset()
        ...
        while cap.isOpened():
            ...
            #sample every 10 frames to keep the app highly responsive
            if frame_count % 10 == 0:
